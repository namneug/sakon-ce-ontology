...
# Preserve FUSEKI_BASE across restarts so TDB2 data persists.
# Only create directories if they don't exist yet.
...
# ==========================================
# Step 3b: Create dataset via admin API (TDB2) if it doesn't exist
# ==========================================
...
# ==========================================
# Step 4: Check existing data — only load if dataset is empty
# ==========================================
...
if [ "$TRIPLE_COUNT" -gt 0 ] 2>/dev/null; then
    echo "  Dataset already has ${TRIPLE_COUNT} triples — skipping data load."
else
    echo "  Dataset is empty — loading ontology data..."
    ...
    if [ "$LOAD_OK" = false ]; then
        echo "[ERROR] Data loading had errors!"
        exit 1
    fi
    ...
    if [ "$TRIPLE_COUNT" -eq 0 ] 2>/dev/null; then
...
echo "============================================"
echo "  Startup complete! ${TRIPLE_COUNT} triples in dataset."
